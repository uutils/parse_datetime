    ...
    pub(super) fn build(self) -> Option<Zoned> {
        ...
        if let Some(date::Date { year, month, day }) = self.date {
            dt = dt
                .with()
                .date(civil::date(
                    year.map(|x| x as i16).unwrap_or(dt.year()),
                    month as i8,
                    day as i8,
                ))
                .build()
        ...
        for rel in self.relative {
            if let relative::Relative::Months(x) = rel {
                // *NOTE* This is done in this way to conform to GNU behavior.
                let days = dt.date().last_of_month().day() as i32;
                dt = dt.checked_add(Span::new().try_days(days * x).ok()?).ok()?;
                continue;
            } else {
                dt = dt.checked_add::<Span>(rel.try_into().ok()?).ok()?;
            }
        }
